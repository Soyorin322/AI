...
try:
    import fcntl
except ImportError:  # Windows has no fcntl; file-lock singleton guard is skipped
    fcntl = None
import logging
import os
import signal
...
def _acquire_file_lock():
    if fcntl is None:  # Windows: no flock available, run without the singleton guard
        logger.info("fcntl unavailable (Windows); skipping file-lock singleton guard")
        return True
